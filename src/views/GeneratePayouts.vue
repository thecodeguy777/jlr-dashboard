...
function getThisWeekSaturday(date = new Date()) {
  const d = new Date(date);
  const day = d.getDay();
  const diff = 6 - day; // Saturday is day 6
  d.setDate(d.getDate() + diff);
  d.setHours(0, 0, 0, 0);
  return d;
}
const selectedSaturday = ref(getThisWeekSaturday());
...
async function commitPayouts() {
  loading.value = true;
  const inserts = previews.value.filter(p => !p.exists).map(async p => {
...
watch(selectedSaturday, (newVal) => {
  selectedSaturday.value = getThisWeekSaturday(newVal);
