...
<template>
  <div
    id="delivery-form"
    class="backdrop-blur-md p-6 rounded-2xl shadow-md border transition-all"
    :class="editMode ? 'bg-yellow-100/10 border-yellow-400' : 'bg-white/10 border-white/10'"
  >
    ...
    <form @submit.prevent="emit('submit', props.formData)">
      <div class="space-y-4">
        ...
        <div>
          ...
          <select v-model="props.formData.worker_id" class="w-full p-2 rounded bg-gray-700 text-white border border-gray-600">
        ...
        <div>
          ...
          <select v-model="props.formData.product_id" class="w-full p-2 rounded bg-gray-700 text-white border border-gray-600">
